<template>
  <v-container align="center" class="fill-height">
    ...
    <v-table density="compact" fixed-header height="35em">
      ...
      <tbody>
        <tr v-for="item in userDataSliced" :key="item['id']">
...
export default {
  ...
  setup() {
    ...
    const numOfRecords = computed(() =>
      Math.ceil(userData.value?.length / rowsSelected.value)
    );
    const userDataSliced = computed(() =>
      userData.value.slice(
        rowsSelected.value * (page.value - 1),
        rowsSelected.value * page.value
      )
    );
    return {
      page,
