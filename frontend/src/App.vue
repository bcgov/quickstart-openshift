...
import { computed, nextTick, onMounted, ref, watch, type ComputedRef, type Ref, type WritableComputedRef } from 'vue';
...
/** Toggle Dark mode */
const isDark: ComputedRef<string> = computed(() => (configStore._themeDark ? 'dark' : 'light'));

/** Theme Color (Sync browser theme color to vuetify theme color) */
const themeColor: ComputedRef<string> = computed(() => theme.computedThemes.value[isDark.value].colors.primary);
