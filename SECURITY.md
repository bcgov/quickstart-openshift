...
| **V14.2** | Secret Management | Separates database connection credentials, API keys, and environment-specific configs from code. | Configuration materialized via OpenShift Secrets templates in [common/openshift.init.yml](./common/openshift.init.yml). |
| **V14.2** | Local Secret Leak Prevention | Prevents accidental commits of staged secrets, API keys, and tokens during local development. | Opt-in [Gitleaks](https://github.com/gitleaks/gitleaks) scan of staged changes via [.githooks/pre-commit](./.githooks/pre-commit), enabled per clone with `git config core.hooksPath .githooks`. |
...
| **V14.4** | HTTP Secure Headers | Strips identifying `Server` headers and enforces secure HTTP response headers (CSP, HSTS, X-Frame-Options, same-origin, and MIME sniffing blocks). | Configured natively in the frontend [frontend/Caddyfile](./frontend/Caddyfile) and verified by weekly OWASP ZAP scans. |
| **V14.4** | Container Hardening | Restricts container execution permissions and prevents host system modifications. | Enforces `readOnlyRootFilesystem: true`, `runAsNonRoot: true`, `allowPrivilegeEscalation: false`, drop all `capabilities`, and default `seccompProfile` in [backend/openshift.deploy.yml](./backend/openshift.deploy.yml) and [frontend/openshift.deploy.yml](./frontend/openshift.deploy.yml). |
| **V14.4** | Network Segmentation | Controls pod communication, isolating network traffic between frontend, backend, and database tiers. | Hardened [NetworkPolicies](https://kubernetes.io/docs/concepts/services-networking/network-policies/) defined in [common/openshift.init.yml](./common/openshift.init.yml). |
