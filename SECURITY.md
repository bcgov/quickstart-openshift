...
| **V14.2** | Local Secret Leak Prevention | Prevents accidental commits of staged secrets, API keys, and tokens during local development. | In-repository [Gitleaks](https://github.com/gitleaks/gitleaks) pre-commit hook via `.githooks/pre-commit` and optional client-side [agent-guardrails](https://github.com/bcgov/agent-guardrails). |
